        ...
        <div className="space-y-2 sm:space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-center gap-1 sm:gap-4">
            <div className="flex items-center justify-center">
              <span className="font-medium text-gray-700 dark:text-gray-300 text-responsive-sm">
                あなたの回答:
              </span>
              <span
                className={`ml-2 font-bold text-responsive-sm ${answer.isCorrect ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}
              >
                {getUserAnswerText()}
              </span>
            </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          {question.options.map((option, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-start space-x-2 sm:space-x-3 p-2 sm:p-3 bg-white dark:bg-gray-700 rounded-lg"
              >
                <span
                  className={`
        ...
        <div className="card-base bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
          ...
          <div className="space-y-2 sm:space-y-3">
            {question.learningResources.map((resource, index) => (
              <div
                key={index}
                className="flex items-start space-x-2 sm:space-x-3 p-2 sm:p-3 bg-white dark:bg-blue-800/30 rounded-lg"
              >
                <span
                  className={`
                  inline-flex items-center px-2 py-1 text-xs font-medium rounded-full flex-shrink-0
                ...
                <svg
                  className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 dark:text-gray-500 flex-shrink-0 mt-1"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
